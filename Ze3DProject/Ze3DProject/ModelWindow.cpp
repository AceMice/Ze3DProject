...
ModelWindow::ModelWindow(const ModelWindow& originalObj)
{
...
bool ModelWindow::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, int screenWidth, int screenHeight)
{
	VertexSimple* vertices = nullptr;
	...
	vertices = new VertexSimple[this->vertexCount];
	if (!vertices) {
		return false;
	}
	...
	vertexBufferDesc.ByteWidth = sizeof(VertexSimple) * this->vertexCount;
...
void ModelWindow::Render(ID3D11DeviceContext* deviceContext)
{
	unsigned int stride;
	unsigned offset;
...
int ModelWindow::GetIndexCount()
{
	return this->indexCount;
}

void ModelWindow::GetWorldMatrix(XMMATRIX& worldMatrix)
{
	worldMatrix = this->worldMatrix;
	return;
}

void ModelWindow::SetWorldMatrix(XMMATRIX worldMatrix)
{
